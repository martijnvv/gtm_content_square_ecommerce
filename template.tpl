___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "displayName": "Contentsquare - E-commerce data (unofficial)",
  "description": "Collect transaction data for your Contentsquare instance for e-commerce tracking.",
  "categories": [
    "ANALYTICS"
  ],
  "securityGroups": [],
  "categories": ["ANALYTICS"],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/png;base64,",
    "displayName": "Martijn van Vreeden",
    "id": "github.com_contentsquare-ps"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "SELECT",
    "name": "eventType",
    "displayName": "Name of the event",
    "macrosInSelect": true,
    "selectItems": [
      {
        "value": "purchase",
        "displayValue": "Purchase"
      },
      {
        "value": "add_to_cart",
        "displayValue": "Add to Cart"
      }
    ],
    "simpleValueType": true
  },
  {
    "type": "CHECKBOX",
    "name": "merchandising",
    "checkboxText": "Enable the merchandising feature",
    "simpleValueType": true
  },
  {
    "displayName": "Purchase parameters",
    "name": "MandatoryParameters",
    "groupStyle": "ZIPPY_OPEN",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "The parameters in this section are required in order to collect transaction data.\n \nPlease make sure to assign variables that get populated on the confirmation page.",
        "name": "label1",
        "type": "LABEL"
      },
      {
        "macrosInSelect": true,
        "selectItems": [],
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "displayName": "Transaction ID",
        "simpleValueType": true,
        "name": "transactionId",
        "type": "SELECT"
      },
      {
        "macrosInSelect": true,
        "selectItems": [],
        "displayName": "Revenue",
        "simpleValueType": true,
        "name": "transactionRevenue",
        "type": "SELECT"
      }
    ]
  },
  {
    "displayName": "Optional parameters",
    "name": "OptionalParameters",
    "groupStyle": "ZIPPY_CLOSED",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "The parameters in this section are optional and are not required to collect transaction data. Feel free to leave them empty if you don\u0027t have available variables or if this approach doesn\u0027t apply to your site.",
        "name": "label2",
        "type": "LABEL"
      },
      {
        "notSetText": "",
        "macrosInSelect": true,
        "selectItems": [
          {
            "value": "eur",
            "displayValue": "EUR"
          },
          {
            "value": "usd",
            "displayValue": "USD"
          }
        ],
        "displayName": "ISO currency code",
        "simpleValueType": true,
        "name": "transactionCurrency",
        "type": "SELECT"
      }
    ]
  },
  {
    "displayName": "Items parameters",
    "name": "Item mapping",
    "groupStyle": "ZIPPY_CLOSED",
    "type": "GROUP",
    "subParams": [
      {
        "displayName": "\u003cstrong\u003eThis is an optional functionality. \u003c/strong\u003e \u003cbr/\u003eItem data is only required for to leverage the \u003ca href\u003d\"https://docs.contentsquare.com/en/web/sending-ecommerce-commands/#sending-a-transaction-for-merchandising\"\u003eMerchandising feature\u003c/a\u003e.\u003cbr/\u003e\u003cbr/\u003eItems parameters can be included on both events, add-to-cart and purchase. They can be mapped from a standard GA4 items object or from any other object inside an array. If you select \u003cem\u003ecustom array\u003c/em\u003e, add the array name as an input to map the parameters to the ContentSquare object.",
        "name": "label3",
        "type": "LABEL"
      },
      {
        "type": "RADIO",
        "name": "itemsArray",
        "displayName": "Choose the products array format",
        "radioItems": [
          {
            "value": "ga4Format",
            "displayValue": "GA4 Standard format"
          },
          {
            "value": "customFormat",
            "displayValue": "Custom array"
          }
        ],
        "simpleValueType": true
      },
      {
        "type": "TEXT",
        "name": "customObject",
        "displayName": "Define the custom array name you wish to use",
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "itemsArray",
            "paramValue": "customFormat",
            "type": "EQUALS"
          }
        ],
        "help": "the standard GA4 format is ecommerce.items"
      },
      {
        "type": "TEXT",
        "name": "sku",
        "displayName": "Product SKU",
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "itemsArray",
            "paramValue": "customFormat",
            "type": "EQUALS"
          }
        ],
        "help": "Required for add-to-cart and purchase event"
      },
      {
        "type": "TEXT",
        "name": "name",
        "displayName": "Product name",
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "itemsArray",
            "paramValue": "customFormat",
            "type": "EQUALS"
          }
        ],
        "help": "Only required for purchase event"
      },
      {
        "type": "TEXT",
        "name": "price",
        "displayName": "Product price",
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "itemsArray",
            "paramValue": "customFormat",
            "type": "EQUALS"
          }
        ],
        "help": "Only required for purchase event"
      },
      {
        "type": "TEXT",
        "name": "quantity",
        "displayName": "Product quantity",
        "simpleValueType": true,
        "help": "Only required for purchase event",
        "enablingConditions": [
          {
            "paramName": "itemsArray",
            "paramValue": "customFormat",
            "type": "EQUALS"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "merchandising",
        "paramValue": true,
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Debug Options",
    "displayName": "Debug Options",
    "groupStyle": "ZIPPY_CLOSED",
    "subParams": [
      {
        "type": "CHECKBOX",
        "name": "debug",
        "checkboxText": "Enable log console for debugging",
        "simpleValueType": true
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const copyFromWindow = require ('copyFromWindow');
const copyFromDataLayer = require('copyFromDataLayer');
const setInWindow = require('setInWindow');
const createQueue = require ('createQueue');
const math = require('Math');
const log = require('logToConsole');
const getType = require('getType');
const itemsArray = data.itemsArray;
const eventType = data.eventType;
const transactionId = data.transactionId;
const transactionRevenue = data.transactionRevenue;
const transactionCurrency = data.transactionCurrency;
const customObject = data.customObject;
const merchandising = data.merchandising;

let _uxaPush = createQueue('_uxa');

if(eventType === 'purchase'){
  log('starting content square purchase event');
_uxaPush(['ec:transaction:create', {
        'id': transactionId,               // mandatory  -  Transaction ID
        'revenue': transactionRevenue,   // mandatory  -  Transaction's total cost
        'currency': transactionCurrency
    }]);
  if (itemsArray === 'ga4Format' && data.merchandising) {
  const mapProductData = i => {
    return {
      id: transactionId,
      sku: i.item_id,
      price: i.price,
      quantity: i.quantity ? math.round(i.quantity) : i.quantity,
      name: i.item_name,
      category: i.item_category || '' // Add category if available
    };
  };

  const eec = copyFromDataLayer('ecommerce', 1) || {};
  if (getType(eec.items) === 'array') {
    eec.items.forEach(i => {
      const product = mapProductData(i);
      _uxaPush(['ec:transaction:items:add', product]);
    });
  }
} else if (itemsArray === 'customFormat' && data.merchandising) {
  const mapProductData = i => {
    return {
      id: i.transactionId,
      sku: i.sku,
      price: i.price,
      quantity: i.quantity ? math.round(i.quantity) : i.quantity,
      name: i.item_name,
      category: i.item_category || '' // Add category if available
    };
  };

  const eec = copyFromDataLayer(customObject, 1) || {};
  if (getType(eec) === 'array') {
    eec.forEach(i => {
      const product = mapProductData(i);
      _uxaPush(['ec:transaction:items:add', product]);
    });
  }
}

_uxaPush(['ec:transaction:send']);

// add to cart event start
} else if(eventType === 'add_to_cart'){
// Function to add product to cart
log('starting content square add-cart event');
  _uxaPush(['trackPageEvent', 'addToCart']);

if(merchandising){
const addProductToCart = product => {
  
  _uxaPush(['ec:cart:add', {
    
      sku: product.sku
    
  }]);
};

if (itemsArray === 'ga4Format') {
  const eec = copyFromDataLayer('ecommerce', 1) || {};
  if (getType(eec.items) === 'array') {
    eec.items.forEach(i => {
      addProductToCart({ sku: i.item_id });
    });
  }
} else if (itemsArray === 'customFormat') {
  const eec = copyFromDataLayer(customObject, 1) || {};
  if (getType(eec) === 'array') {
    eec.forEach(i => {
      addProductToCart({ sku: i.sku });
    });
  }
}
}else{
  _uxaPush(['ec:cart:add', {    
  }]);
}
}  
data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_uxa"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "read_data_layer",
        "versionId": "1"
      },
      "param": [
        {
          "key": "allowedKeys",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Developed by Martijn van Vreeden based on the basic template by Uri Gobey @ Contentsquare


